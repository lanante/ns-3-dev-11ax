    ...
    case WifiPhyState::SLEEP:
      {
        ...
        for (uint8_t i = 0; i < std::max (1, channelWidth / 20); i++)
          {
            auto band = GetBand (((channelWidth > 20) ? 20 : channelWidth), i);
            bool isPrimary = (i == GetPrimaryBandIndex (20));
...
WifiPhy::StartReceiveHeader (Ptr<Event> event)
{
  ...
  double snr = m_interference.CalculateSnr (m_currentEvent, channelWidth, GetBand (channelWidth, GetPrimaryBandIndex (channelWidth)));
...
{
  NS_ASSERT (channelWidth <= GetChannelWidth ());
  Time delaySinceIdle = Seconds (0);
  uint8_t startIndex;
  uint8_t stopIndex;
  if (channelWidth == GetChannelWidth ())
    {
       startIndex = 1;
       stopIndex = std::max (1, channelWidth / 20);
    }
  else
    {
      startIndex = GetPrimaryBandIndex (channelWidth) + 1;
      stopIndex = startIndex + (channelWidth / 20);
    }
  for (uint8_t i = startIndex; i < stopIndex; i++)
    {
      auto band = GetBand (((channelWidth > 20) ? 20 : channelWidth), i);
      delaySinceIdle = std::max (delaySinceIdle, m_state->GetDelaySinceIdle (band));
    }
