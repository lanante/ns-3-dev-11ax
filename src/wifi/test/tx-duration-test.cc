                                     ...
                                     Time knownDuration)
{
  ...
  Ptr<YansWifiPhy> phy = CreateObject<YansWifiPhy> ();
  std::list<uint16_t> testedFrequencies {CHANNEL_36_MHZ, CHANNEL_1_MHZ}; //Durations vary depending on frequency; test also 2.4 GHz (bug 1971)
  for (auto & testedFrequency : testedFrequencies)
    {
      if (testedFrequency == CHANNEL_1_MHZ)
        {
          knownDuration += MicroSeconds (6);
        }
      ...
      Time calculatedDurationUsingList = CalculateTxDurationUsingList (sizes, staIds, txVector, testedFrequency);
      if (calculatedDuration != knownDuration || calculatedDuration != calculatedDurationUsingList)
        {
          std::cerr << "size=" << longuestSize
                    << " freq=" << testedFrequency
                    << " staId=" << staId
                                              ...
                                              WifiTxVector txVector, uint16_t frequency)
{
  ...
  std::map<uint16_t, Ptr<const WifiPsdu>> psduMap;
  ...
  return WifiPhy::CalculateTxDuration (psduMap, txVector, frequency);
}
...
HeSigBDurationTest::DoRun (void)
{
  ...
  NS_TEST_EXPECT_MSG_EQ (WifiPhy::GetPlcpSigBDuration (txVector), MicroSeconds (4), "HE-SIG-B should only last one OFDM symbol");
  ...
  NS_TEST_EXPECT_MSG_EQ (WifiPhy::GetPlcpSigBDuration (txVector), MicroSeconds (8), "HE-SIG-B should last two OFDM symbols");
  ...
  NS_TEST_EXPECT_MSG_EQ (WifiPhy::GetPlcpSigBDuration (txVector), MicroSeconds (16), "HE-SIG-B should last four OFDM symbols");
  ...
  NS_TEST_EXPECT_MSG_EQ (WifiPhy::GetPlcpSigBDuration (txVector), MicroSeconds (20), "HE-SIG-B should last five OFDM symbols");
}
