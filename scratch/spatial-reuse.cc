...
uint32_t nBss = 1; // number of BSSs.  Can be 1 or 2 (default)
...
bool fixedPosition=false;
  ...
  cmd.AddValue ("fixedPosition", "Fixed STA location relative to AP", fixedPosition);
...
Vector v;
      for (uint32_t i = 0; i < n; i++)
        {
        if (fixedPosition)
                 v = Vector (x1, y1+r, 0.0);
        else
                 v = unitDiscPositionAllocator1->GetNext ();
          positionAlloc->Add (v);
      ...
      if(nBss>=2)
        {
                ...
                for (uint32_t i = 0; i < n; i++)
                        {
        if (fixedPosition)
                 v = Vector (x2, y2+r, 0.0);
        else
                 v = unitDiscPositionAllocator2->GetNext ();
          positionAlloc->Add (v);
      ...
      for (uint32_t i = 0; i < n; i++)
        {
        if (fixedPosition)
                 v = Vector (x3, y3+r, 0.0);
        else
                 v = unitDiscPositionAllocator3->GetNext ();
          positionAlloc->Add (v);
      ...
      for (uint32_t i = 0; i < n; i++)
        {
        if (fixedPosition)
                 v = Vector (x4, y4+r, 0.0);
        else
                 v = unitDiscPositionAllocator4->GetNext ();
          positionAlloc->Add (v);
    ...
    // Network "E"
      ...
      for (uint32_t i = 0; i < n; i++)
        {
        if (fixedPosition)
                 v = Vector (x5, y5+r, 0.0);
        else
                 v = unitDiscPositionAllocator5->GetNext ();
          positionAlloc->Add (v);
      ...
      for (uint32_t i = 0; i < n; i++)
        {
        if (fixedPosition)
                 v = Vector (x6, y6+r, 0.0);
        else
             v = unitDiscPositionAllocator6->GetNext ();
          positionAlloc->Add (v);
        }
        }
      ...
      for (uint32_t i = 0; i < n; i++)
        {
        if (fixedPosition)
            v = Vector (x7, y7+r, 0.0);
        else
            v = unitDiscPositionAllocator7->GetNext ();
          positionAlloc->Add (v);
